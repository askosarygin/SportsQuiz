...
class FragmentScreenWallpaper: SportsQuizFragment(R.layout.fragment_screen_wallpaper) {
    ...
    override fun onCreateView(
        ...
        val wallpaper = getBundleNavigation(resources.getString(com.example.common.R.string.bundle_key_wallpaper)) as Wallpaper
        ...
        viewModel.model.collectWithOld(lifecycleScope) { oldModel, newModel ->
            ...
            if (oldModel?.buttonBuyVisible != newModel.buttonBuyVisible) {
                if (newModel.buttonBuyVisible) {
                    binding.btnBuyOrSetWallpaper.text = resources.getText(com.example.common.R.string.buy)
                    ...
                    binding.btnBuyOrSetWallpaper.text = resources.getText(com.example.common.R.string.set_wallpaper_to_phone)
                    binding.btnBuyOrSetWallpaper.setOnClickListener {
            ...
            if (oldModel?.name != newModel.name) {
                val wallpaperName = "${resources.getString(com.example.common.R.string.wallpaper)} ${newModel.name}"
                binding.tvWallpaperName.text = wallpaperName
            }
            if (oldModel?.price != newModel.price) {
                if (newModel.price != -1) {
                    val wallpaperPrice = "${newModel.price} ${resources.getString(com.example.common.R.string.points_price)}"
                    binding.tvWallpaperPrice.text = wallpaperPrice
                } else {
                    val wallpaperPriceBought = resources.getString(com.example.common.R.string.bought)
